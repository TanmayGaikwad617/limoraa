...
async function loadCollectionVideos(client: PoolClient, userId: string, collectionId: string) {
...
async function reindexCollectionVideos(client: PoolClient, userId: string, collectionId: string) {
...
async function loadCollectionDetail(client: PoolClient, userId: string, collectionId: string) {
  ...
  app.post("/collections/:id/videos", async (request) => {
    ...
    return withRlsUser(auth.user, async (client) => {
      ...
      const videoResult = await client.query<{ id: string }>(
        `select id from public.videos where id = $1 and user_id = $2`,
        [videoId, auth.user.id],
      );
