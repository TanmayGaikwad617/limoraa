...
function buildVideoSelect(whereClause: string, orderClause: string): string {
  return `
    select
      v.id,
      v.user_id,
...
export async function loadVideoById(
  client: PoolClient,
  videoId: string,
): Promise<HydratedVideo | null> {
  const result = await client.query<VideoBaseRow>(
    buildVideoSelect("where v.id = $1", "limit 1"),
    [videoId],
  );
...
export async function loadVideos(
  ...
  const clauses: string[] = ["v.user_id = $1"];
  let index = 2;
  ...
  if (query.creator) {
    clauses.push(
      `(lower(coalesce(v.creator_name, '')) like lower($${index}) or lower(coalesce(v.creator_handle, '')) like lower($${index}))`,
    );
  ...
  const result = await client.query<VideoBaseRow>(sql, values);
  return hydrateVideos(client, result.rows);
}

async function insertSaveUsage(
  client: PoolClient,
  userId: string,
  videoId: string,
...
async function saveVideoForUser(userId: string, rawUrl: string) {
  const normalized = normalizeVideoUrl(rawUrl);

  const saved = await withRlsUser({ id: userId }, async (client) => {
    const existingResult = await client.query<VideoBaseRow>(
      buildVideoSelect("where v.normalized_url = $1", "limit 1"),
    ...
    const insertResult = await client.query<VideoBaseRow>(
      `
...
async function enqueueIndexJob(userId: string, videoId: string): Promise<void> {
