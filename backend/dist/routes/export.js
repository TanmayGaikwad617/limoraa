...
async function loadAllVideos(client, userId) {
    const result = await client.query(buildVideoSelect("where v.user_id = $1"), [
        userId,
    ]);
    return hydrateVideos(client, result.rows);
}
async function loadAllCollections(client, userId) {
