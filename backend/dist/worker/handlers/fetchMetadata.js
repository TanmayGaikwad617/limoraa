...
function buildEmptyMetadata(video) {
...
function buildMetadataFromYouTube(video, metadata) {
...
function buildMetadataFromInstagram(video, metadata) {
    const fallback = buildEmptyMetadata(video);
    if (!metadata) {
        return fallback;
    }
...
function buildMetadataFromTwitter(video, metadata) {
    const fallback = buildEmptyMetadata(video);
    if (!metadata) {
        return fallback;
    }
...
async function fetchProviderMetadata(video) {
...
async function loadVideoForMetadata(client, videoId) {
...
export async function registerFetchMetadataHandler(boss) {
    await boss.work(FETCH_METADATA_QUEUE, async (jobs) => {
        const job = jobs[0];
        if (!job) {
            return;
        }
