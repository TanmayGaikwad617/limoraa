...
function toMetadata(input) {
    return {
        ...
        hashtags: Array.isArray(input.hashtags)
            ? input.hashtags.filter((value) => typeof value === "string").map((value) => trimText(value, 100))
            : [],
...
function computeQualityScore(metadata) {
    let score = 0;
    if (metadata.title) {
...
function computeConfidenceCap(qualityScore) {
...
function inferIntent(metadata, format) {
    const text = getTextBlob(metadata);
...
function inferAudience(topic, metadata) {
    const text = getTextBlob(metadata);
...
function inferVerticalType(topic, format, metadata) {
    const text = getTextBlob(metadata);
...
function inferVerticalFields(verticalType, metadata, topic) {
    const text = getTextBlob(metadata);
    switch (verticalType) {
        case "recipe":
            return {
...
function extractCandidateTags(text) {
    return text
        .toLowerCase()
        .split(/[^a-z0-9]+/)
...
function sanitizeTags(tags, metadata) {
...
function buildSummary(metadata, topic) {
    void topic;
...
function sanitizeVerticalFields(verticalType, rawValue, metadata, topic) {
    const fallback = inferVerticalFields(verticalType, metadata, topic);
    const record = readObject(rawValue);
    const readString = (key) => trimText(typeof record[key] === "string" ? record[key] : "", 120);
    switch (verticalType) {
        case "recipe":
            return {
...
function normalizeAnalysis(metadata, raw) {
...
export async function analyzeVideoMetadata(input) {
