...
async function fetchInstagramOEmbed(originalUrl, appToken) {
    ...
    try {
        ...
        console.warn(`Failed to fetch Instagram oEmbed data: ${error instanceof Error ? error.message : String(error)}`);
        return null;
    }
    if (!response.ok) {
    ...
    try {
        ...
        console.warn(`Failed to parse Instagram oEmbed response: ${error instanceof Error ? error.message : String(error)}`);
        return null;
    }
}
