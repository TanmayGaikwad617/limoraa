...
async function fetchOEmbed(originalUrl) {
    ...
    try {
        ...
        console.warn(`Failed to fetch Twitter oEmbed data: ${error instanceof Error ? error.message : String(error)}`);
        return null;
    }
    if (!response.ok) {
    ...
    try {
        ...
        console.warn(`Failed to parse Twitter oEmbed response: ${error instanceof Error ? error.message : String(error)}`);
        return null;
    }
}
async function fetchApiV2(tweetId) {
    ...
    try {
        ...
        console.warn(`Failed to fetch Twitter API v2 data: ${error instanceof Error ? error.message : String(error)}`);
        return null;
    }
    if (!response.ok) {
        if (response.status === 429) {
            ...
            console.warn("Twitter API v2 unavailable with status 402; falling back to oEmbed for this process");
        }
        else {
    ...
    try {
        ...
        console.warn(`Failed to parse Twitter API v2 response: ${error instanceof Error ? error.message : String(error)}`);
        return null;
    }
}
export async function parseTwitter(url) {
    ...
    const apiHashtags = apiData?.entities?.hashtags?.map((hashtag) => hashtag.tag).filter((tag) => typeof tag === "string" && tag.trim().length > 0) ?? [];
    return {
