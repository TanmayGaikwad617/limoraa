...
function stripTrackingParams(url) {
    const paramsToRemove = Array.from(url.searchParams.keys()).filter((key) => key.startsWith("utm_") || key === "fbclid" || key === "igshid" || key === "si");
...
function canonicalizeInstagram(url) {
...
function canonicalizeTiktok(url) {
    const segments = url.pathname.split("/").filter(Boolean);
